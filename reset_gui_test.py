...
root = Tk()
root.title("Dif Reset Tool")

Username = ("5000")



def WDipad_steps_non_liquor(store_num_string):  #Opens WDIpad, enters the store num, clicks SSH and clicks CC
...
def Wdipad_steps_liquor(store_num_string):  #Opens the CC for liquor stores from WDIpad
        os.chdir(r"C:\Users\gouldj\Desktop\Programming Projects\CC automation")
        try:
            a1, a2 = pyautogui.locateCenterOnScreen('imgs/testwdimg.png', confidence=0.9)   
            pyautogui.click(a1, a2) 
...
def start():
    ...
    stores = [2, 3, 5, 7, 8, 12, 18, 19, 25, 28, 30, 32, 40, 51, 54, 60, 67, 68, 70, 77, 81, 84, 85, 86, 93, 94, 96, 103, 104, 106,
            ...
            599, 606, 607, 608, 609, 611, 612, 619, 630, 631, 632, 639, 640, 644, 648, 649, 652, 655, 656, 657, 658, 664,
    ...
    for n in Store_Input_List_Int:  #running the whole script for each store number in the list that was entered
        ...
        if n in stores:
            WDipad_steps_non_liquor(store_num_string)
        else:
            Wdipad_steps_liquor(store_num_string)
...
label1 = Label(root, text="Enter store numbers:")
store_input = Entry(root, width=50, borderwidth=5)
submit_button = Button(root, text="Reset Dif", command=start)
label2 = Label(root, text="Enter daily password:")
password_input = Entry(root, width=50, borderwidth=5)


#Placing the different elements onto the GUI
label1.grid(row=0, column=0, padx=25, pady=5, columnspan=1)
store_input.grid(row=0, column=1, padx=25, pady=30, columnspan=1)
submit_button.grid(row=2, column=0,padx=0, pady=20)
label2.grid(row=1, column=0, padx=25, pady=5, columnspan=1)
password_input.grid(row=1, column=1, padx=25, pady=30, columnspan=1)
